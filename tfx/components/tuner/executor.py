...
from tfx.types.standard_component_specs import BEST_HYPERPARAMETERS_KEY
from tfx.types.standard_component_specs import TUNE_ARGS_KEY
...
def get_tune_args(
  ...
  tune_args = exec_properties.get(TUNE_ARGS_KEY)
  if not tune_args:
    return None
...
def write_best_hyperparameters(
  ...
  best_hparams_path = os.path.join(
      artifact_utils.get_single_uri(
          output_dict[BEST_HYPERPARAMETERS_KEY]),
      _DEFAULT_FILE_NAME)
