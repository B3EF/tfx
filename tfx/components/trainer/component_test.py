...
from tfx.types.standard_component_specs import RUN_FN_KEY
...
class ComponentTest(tf.test.TestCase):
  ...
  def _verify_outputs(self, trainer):
    self.assertEqual(standard_artifacts.Model.TYPE_NAME,
                     trainer.outputs[MODEL_KEY].type_name)
    self.assertEqual(standard_artifacts.ModelRun.TYPE_NAME,
                     trainer.outputs[MODEL_RUN_KEY].type_name)
  ...
  def testConstructFromModuleFile(self):
    ...
    self.assertEqual(module_file, trainer.spec.exec_properties[MODULE_FILE_KEY])
  ...
  def testConstructWithParameter(self):
    ...
    self.assertJsonEqual(
        str(module_file), str(trainer.spec.exec_properties[MODULE_FILE_KEY]))
  ...
  def testConstructFromTrainerFn(self):
    ...
    trainer = component.Trainer(
        ...
        eval_args=self.eval_args)
    self._verify_outputs(trainer)
    self.assertEqual(trainer_fn, trainer.spec.exec_properties[TRAINER_FN_KEY])
  ...
  def testConstructFromRunFn(self):
    ...
    trainer = component.Trainer(
        ...
        eval_args=self.eval_args)
    self._verify_outputs(trainer)
    self.assertEqual(run_fn, trainer.spec.exec_properties[RUN_FN_KEY])
  ...
  def testConstructWithHParams(self):
    ...
    self.assertEqual(standard_artifacts.HyperParameters.TYPE_NAME,
                     trainer.inputs[HYPERPARAMETERS_KEY].type_name)
